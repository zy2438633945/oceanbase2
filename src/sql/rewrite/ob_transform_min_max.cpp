...
namespace oceanbase
{
  namespace sql
  {
                                                    ...
                                                    bool &is_valid)
    {
      ...
      else if (select_stmt->get_from_item_size() != 1 || select_stmt->get_from_item(0).is_joined_ || select_stmt->get_aggr_item_size() < 1 || !select_stmt->is_scala_group_by() || select_stmt->is_contains_assignment() || select_stmt->get_aggr_item_size() > 1)
      {
    ...
    int ObTransformMinMax::do_minmax_transform(ObSelectStmt *select_stmt)
    {
      ...
      else
      {
        ...
        {
          if (OB_ISNULL(aggr_expr = select_stmt->get_aggr_item(i)) || OB_ISNULL(aggr_param = aggr_expr->get_param_expr(0)))
          {
            ret = OB_ERR_UNEXPECTED;
          ...
          else if (OB_ISNULL(query_ref_expr) || OB_ISNULL(child_stmt) || OB_ISNULL(target_expr = child_stmt->get_select_item(0).expr_))
          {
            ret = OB_ERR_UNEXPECTED;
                                                       ...
                                                       ObSelectStmt *&child_stmt)
    {
      ...
      if (OB_ISNULL(ctx_) || OB_ISNULL(ctx_->expr_factory_) || OB_ISNULL(ctx_->stmt_factory_) || OB_ISNULL(ctx_->allocator_) || OB_ISNULL(aggr_param))
      {
        ret = OB_INVALID_ARGUMENT;
      ...
      else if (new_aggr_param_index < 0 || new_aggr_param_index >= new_select_exprs.count() || OB_ISNULL(new_aggr_param = new_select_exprs.at(new_aggr_param_index)))
      {
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("failed to find new aggr param", K(ret));
      }
